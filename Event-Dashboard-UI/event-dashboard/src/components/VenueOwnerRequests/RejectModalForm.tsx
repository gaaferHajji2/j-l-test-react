...
const RejectReasonModal = ({ isOpen, venueName, onClose, onConfirm }) => {
  ...
  const validate = () => {
    if (!reason.trim()) {
      setError(t('venueOwnerRequests.rejectReasonRequired'));
      return false;
    }
    if (reason.trim().length < 10) {
      setError(t('venueOwnerRequests.rejectReasonMinLength'));
      return false;
    }
    setError('');
    return true;
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t('venueOwnerRequests.rejectReasonTitle')}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{venueName}</p>
          </div>
        ...
        {/* Body */}
        <form id="venue-owner-reject-form" onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label htmlFor="venueOwnerRejectReason" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
              {t('venueOwnerRequests.rejectReasonLabel')}
            </label>
            <textarea
              id="venueOwnerRejectReason"
              rows={4}
              value={reason}
              onChange={handleChange}
              placeholder={t('venueOwnerRequests.rejectReasonPlaceholder')}
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          >
            ...
            {t('venueOwnerRequests.submitRejectBtn')}
          </button>
          <button
            type="button"
            onClick={handleClose}
          ...
          >
            {t('venueOwnerRequests.cancelRejectBtn')}
          </button>
        </div>
      </div>
    </div>
