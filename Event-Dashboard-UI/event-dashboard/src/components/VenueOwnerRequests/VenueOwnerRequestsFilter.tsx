...
const VenueOwnerRequestsFilter = ({ filters, onFilterChange }) => {
  ...
  const getSortIcon = (field) => {
    ...
    return filters.sortBy.order === 'asc'
      ? <svg className="w-4 h-4 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
      : <svg className="w-4 h-4 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="relative">
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm"
          />
        ...
        >
          <option value="all">{t('venueOwnerRequests.allStatuses')}</option>
        ...
        <div className="flex gap-2">
          <button onClick={() => handleSortChange('submittedAt')} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors text-xs font-medium">
            {t('venueOwnerRequests.submittedAt')}{getSortIcon('submittedAt')}
          </button>
          <button onClick={() => handleSortChange('venueName')} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors text-xs font-medium">
            {t('venueOwnerRequests.venueName')}{getSortIcon('venueName')}
          </button>
        </div>
      </div>
    </div>
...
export default VenueOwnerRequestsFilter;